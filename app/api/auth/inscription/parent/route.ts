...
const inscriptionParentSchema = z.object({
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { nom, prenom, email, telephone, password, codeMosquee } =
      inscriptionParentSchema.parse(body);

    // Vérifier que le code mosquée existe
    // Le code peut être l'ID de la mosquée ou un code spécial
    const mosquee = await prisma.mosquee.findFirst({
      where: {
        OR: [
          { id: codeMosquee },
          // Vous pouvez ajouter un champ codeMosquee dans le modèle Mosquee si nécessaire
        ],
      },
    });
