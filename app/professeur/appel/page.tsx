...
export default function AppelPage() {
  return (
    <Suspense fallback={<div className="py-12 text-center text-sm text-gray-500">Chargement...</div>}>
      <AppelPageContent />
    </Suspense>
  );
}
  ...
  useEffect(() => {
    const fetchClasses = async () => {
      try {
        ...
        const data = await response.json() as { id: string; nom: string; niveau: string }[];
        setClasses(data);
